...
sudo cp "$BUILD_ROOT/rootfs/usr/local/bin/persist-ssh" "$ROOTFS_DIR/usr/local/bin/"
sudo chmod +x "$ROOTFS_DIR/usr/local/bin/persist-ssh"

# Install systemd service
sudo cp "$BUILD_ROOT/rootfs/etc/systemd/system/persist-ssh.service" "$ROOTFS_DIR/etc/systemd/system/"
